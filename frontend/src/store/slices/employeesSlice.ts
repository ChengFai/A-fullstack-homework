...
import { getEmployees, suspendEmployee, activateEmployee } from '../../services/employees'
...
export const fetchEmployees = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error?.response?.data?.detail || error?.message || '获取员工列表失败')
    }
  }
)
...
export const suspendEmployeeAction = createAsyncThunk(
  ...
  async (employeeId: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error?.response?.data?.detail || error?.message || '暂停员工失败')
    }
  }
)
...
export const activateEmployeeAction = createAsyncThunk(
  ...
  async (employeeId: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error?.response?.data?.detail || error?.message || '激活员工失败')
    }
  }
)
